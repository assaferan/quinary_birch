...
TempFile::TempFile(std::string const& eFile)
{
  FileName=eFile;
}

TempFile::TempFile(char* eFile)
{
  FileName=eFile;
}
...
//
bool TempFile::IsExisting() const
{
  return IsExistingFile(FileName);
}

std::string TempFile::string() const
{
  return FileName;
}
...
TempDirectory::TempDirectory(std::string const& eDir)
{
  DirName=eDir;
...
TempDirectory& TempDirectory::operator=(TempDirectory&& eTemp)
{
  DirName=eTemp.str();
  IsInitialized=true;
...
TempDirectory::TempDirectory(TempDirectory && eTemp) : DirName(eTemp.str()), IsInitialized(true)
{
  eTemp.DirName="unset_and_irrelevant";
  eTemp.IsInitialized=false;
}
...
//
bool TempDirectory::IsExisting() const
{
  return IsExistingDirectory(DirName);
}

std::string TempDirectory::str() const
{
  return DirName;
}

CondTempDirectory::CondTempDirectory()
{
  used=false;
  DirName="unset_and_irrelevant";
}

CondTempDirectory::CondTempDirectory(bool const& eUsed, std::string const& eDir)
{
  used=eUsed;
  if (used) {
    DirName=eDir;
...
CondTempDirectory& CondTempDirectory::operator=(CondTempDirectory&& eTemp)
{
  used=eTemp.usedness();
  DirName=eTemp.str();
  eTemp.DirName="unset_and_irrelevant";
...
CondTempDirectory::CondTempDirectory(CondTempDirectory && eTemp) : used(eTemp.usedness()), DirName(eTemp.str())
{
}
...
//
bool CondTempDirectory::IsExisting() const
{
  return IsExistingDirectory(DirName);
}

bool CondTempDirectory::usedness() const
{
  return used;
}

std::string CondTempDirectory::str() const
{
  return DirName;
}
